...
export default class Bank {
  ...
  findBranchByName(branchName: string): Branch[] | null {
    ...
    const targetBranch = this.branches.filter((branch) =>
      branch.getName().toLowerCase().includes(branchName)
    );
    return targetBranch;
  }
    ...
    try {
      if (this.checkBranch(branch)) {
        if (branch.addCustomerTransaction(customerId, amount)) {
          ...
          console.log(
            `Transaction Failed, You do not have an account in ${branch.getName()}`
          );
          return false;
        }
      } else {
